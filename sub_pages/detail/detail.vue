<template>
	<view>
		...
		<view class="flex justify-around fixed padding-tb-sm">
			<button @click="handleAdd(detail,tabIdx)" class="cu-btn bg-brown lg" type="default">加入购物车</button>
...
<script>
	...
	export default {
		...
		methods: {
			// ...mapMutations({  //打包小程序的时候有兼容问题
			// 	'handleAdd':'cart/cartAddMut'
			// }),
			handleAdd(detail,idx){
				this.$store.commit('cart/cartAddMut',{...detail,idx})
			}
		},
	}
</script>
